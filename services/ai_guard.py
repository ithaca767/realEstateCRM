...
def _today() -> date:
    # Ulysses hard-codes NY timezone for datetime-local behavior.
    # These counters are date-based; using server local date is acceptable.
    return date.today()
...
def ensure_ai_allowed_and_reset_if_needed(conn, user_id: int) -> AIUsageSnapshot:
    ...
    Enforces:
      - global AI flag
      - user opt-in
      - daily limit + daily reset
      - monthly cap + monthly reset
    Uses FOR UPDATE to avoid race conditions on counters.
    """
    ...
    last_daily_reset = row["ai_last_daily_reset_at"]  # DATE or None

    monthly_cap = row["ai_monthly_cap_cents"]  # INT or None
    monthly_spend = int(row["ai_monthly_spend_cents"] or 0)
    last_monthly_reset = row["ai_last_monthly_reset_at"]  # DATE or None
    ...
    last_month = _month_key(last_monthly_reset) if last_monthly_reset else None
    ...
    monthly_cap_int: Optional[int] = int(monthly_cap) if monthly_cap is not None else None
    if monthly_cap_int is not None and monthly_spend >= monthly_cap_int:
    ...
    return AIUsageSnapshot(
        ...
        monthly_cap_cents=monthly_cap_int,
    )
...
def increment_ai_usage_on_success(conn, user_id: int, add_spend_cents: int = 0) -> None:
    ...
    Caller should already have called ensure_ai_allowed_and_reset_if_needed()
    within the same transaction, which locks the row.
    """
